...
exports.createPerson = async (req) => {
	try{
		...
		const findedTitle = await titleDal.findById(json.title);
		findedTitle.persons.push(json._id);
		await titleDal.create(findedTitle);

		const findedCompany = await companyDal.findById(json.company);
...
exports.deletePersonById = async (req) => {
	try{
		...
		if(isDeletedAvatar && isDeletedCv){
			...
			const newTitle = findedTitle.persons.filter((item) => item.toString() !== findedPerson._id.toString());
			findedTitle.persons = newTitle;
			await titleDal.create(findedTitle);
			
			const findedCompany = await companyDal.findById(findedPerson.company);
			const newCompany = findedCompany.persons.filter((item) => item.toString() !== findedPerson._id.toString());
			findedCompany.persons = newCompany;
			await companyDal.create(findedCompany);
			return;
