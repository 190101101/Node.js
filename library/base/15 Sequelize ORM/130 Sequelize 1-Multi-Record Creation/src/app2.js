...
router.get('/read', async (req, res) => {
   try{
      const response = await Test.findAll({logging:true});
      console.log(response);
...
router.delete('/delete/:dataId', async (req, res) => {
   ...
   try{
      const findedData = await Test.findOne({where: {test_id: dataId}});
...
router.post('/multiple', async (req, res) => {
   const {data} = req.body;
   try{
      const response = Test.bulkCreate(data);
      res.json(response).status(200);
   }catch(error) {
      res.json({message: 'oops'}).status(500);
   }
...
router.get('/getById/:dataId', async (req, res) => {
      const {dataId} = req.params;
   try{
      const response = await Test.findByPk(dataId);
      res.json(response).status(200);
   }catch(err){
      res.json({message: 'oops'}).status(500);
   }
